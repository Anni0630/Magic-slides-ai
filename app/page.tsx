...
export default function Home() {
  ...
  const handleSend = async () => {
    ...
    setSlides([]); // Clear slides to show clean UI during generation
    setInputText('');
    setIsLoading(true);

    try {
      // Generate slides using Gemini
      const newSlides = await geminiService.generateSlides(userMessage.text, []); // Pass an empty array to slides as we clear them above
      setSlides(newSlides);
  ...
  if (apiError) {
    // ... (API error return JSX remains the same)
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 bg-gray-100 flex items-center justify-center">
  ...
  // Determine if we should show the full-page, clean UI.
  // We show it if:
      ...
      <div className="min-h-screen bg-gray-100 flex items-center justify-center">
        <div className="text-center p-8 ">
          ...
          {/* {messages.length > 0 && (
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 mb-8 max-h-80 overflow-y-auto">
              {messages.map((message) => (
                <div
                  key={message.id}
                  className={`mb-4 p-3 rounded-xl max-w-[90%] ${
                    message.isUser
              ...
              <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
                ...
                <div className="space-y-3 text-sm text-gray-600">
                  <div className="flex items-center space-x-2">
                    ...
                    <span>I've moved past the initial research phase...</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse" style={{ animationDelay: '0.2s' }}></div>
                    <span>I've formulated specific search queries...</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse" style={{ animationDelay: '0.4s' }}></div>
                    <span>I'm now gathering data from several reputable sources...</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-2 flex items-center">
            ...
            <button
              onClick={handleSend}
              disabled={!inputText.trim() || !geminiService || isLoading}
        ...
        {showHistory && (
          // ... (Chat History Sidebar JSX remains the same)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 bg-gray-100">
      <div className="container mx-auto px-4 py-8">
        ...
        {/* Chat History Sidebar (Two-Column View) */}
        {showHistory && (
          // ... (Chat History Sidebar JSX remains the same)
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex">
            <div className="bg-white w-80 h-full overflow-y-auto p-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-6xl mx-auto">
          ...
          {/* Slides Section - THIS REMAINS THE SAME */}
